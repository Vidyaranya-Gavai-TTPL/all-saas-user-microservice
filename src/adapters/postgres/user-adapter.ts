...
export class PostgresUserService implements IServicelocator {
  ...
  async processUserDetails(userSearchDto: UserSearchDto, results) {
    ...
    if (userData && userData.getUserDetails.length > 0) {
      userData.getUserDetails.forEach((user) =>
        results.getUserDetails.push(user)
      );
    }
  }
  ...
  async searchUser(
    ...
    let { limit, offset, filters, sort, tenantCohortRoleMapping } =
      userSearchDto;
    offset = offset || 0;
    limit = limit || 200;
    let results = {
    ...
    try {
      if (isSuperAdmin) {
        ...
        await this.validateUserandTenantMapping(
          userId,
          tenantCohortRoleMapping.tenantId
        );
        ...
        if (userRoles.code === "tenant_admin") {
          ...
          throw new Error(
            "User does not have sufficient permissions for this tenant"
          );
        }
        ...
        if (userRoles.code === "tenant_admin") {
          ...
          await this.validateCohortAdminMapping(
            userId,
            tenantCohortRoleMapping.cohortId
          );
        } else {
          throw new Error(
            "User does not have sufficient permissions for this cohort"
          );
        }
        ...
        if (userRoles.code === "tenant_admin") {
          ...
          throw new Error(
            "User does not have sufficient permissions for this mapping"
          );
        }
      ...
      results.getUserDetails = results.getUserDetails.slice(
        offset,
        offset + limit
      );
  ...
  async findAllUserDetails(userSearchDto) {
    let { limit, offset, filters, exclude, sort, tenantCohortRoleMapping } =
      userSearchDto;
    ...
    if (cohortId && cohortId.length > 0) {
      ...
      whereCondition += `${
        index > 0 ? " AND " : ""
      } CM."cohortId" IN (${cohortCondition})`;
      index++;
    }
  ...
  async getUsersDetailsById(userData: UserData, response: any) {
    ...
    try {
      ...
      // }
      const isSuperAdmin = await this.postgresRoleService.isSuperAdmin(
        userData?.userId
      );
  ...
  async createUser(
    ...
    try {
      ...
      let checkUserinKeyCloakandDb = await this.checkUserinKeyCloakandDb(
        userCreateDto
      );
      ...
      let result = await this.createUserInDatabase(
        request,
        userCreateDto,
        response
      );
      ...
      if (
        ...
        if (customFields) {
          ...
          for (let fieldValues of userCreateDto.customFields) {
            ...
            let res = await this.fieldsService.updateCustomFields(
              userId,
              fieldData,
  ...
  async checkCohort(tenantId: any, cohortData: any) {
    ...
    for (let cohortId of cohortData) {
      let findCohortData = await this.cohortRepository.findOne({
  ...
  public async validateCustomField(userCreateDto, response, apiId) {
    ...
    for (const fieldsData of fieldValues) {
      ...
      let getFieldDetails: any = await this.fieldsService.getFieldByIdes(
        fieldId
      );
      ...
      if (
        ...
        let getOption = await this.fieldsService.findDynamicOptions(
      ...
      let checkValidation = this.fieldsService.validateFieldValue(
        getFieldDetails,
        fieldsData["value"]
      );
    ...
    let context = "USERS";
    let getFieldIds = await this.fieldsService.getFieldIds(
      context,
      contextType
    );
